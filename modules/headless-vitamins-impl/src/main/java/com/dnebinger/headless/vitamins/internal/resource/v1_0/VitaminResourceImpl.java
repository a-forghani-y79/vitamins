...
public class VitaminResourceImpl extends BaseVitaminResourceImpl {
    ...
    protected Vitamin _toVitamin(PersistedVitamin pv) throws PortalException {
        return new Vitamin() {{
            creator = CreatorUtil.toCreator(portal, userLocalService.getUser(pv.getUserId()));
            artcleId = pv.getArticleId();
            ...
            chemicalName = ListUtil.toArray(pv.getChemicalNames(), VALUE_ACCESSOR);
...
//					document -> _toVitamin(
//							persistedVitaminService.getPersistedVitamin(
...
//			throw e;
//		}
        List<Vitamin> list = persistedVitaminService.getAll().stream().map(persistedVitamin -> {
            try {
    ...
    public Vitamin postVitamin(Vitamin v) throws Exception {
        ...
        try {
            PersistedVitamin pv = persistedVitaminService.addPersistedVitamin(
                    v.getId(), v.getName(), v.getGroup(), v.getDescription(), _toTypeCode(v.getType()), v.getArtcleId(), v.getChemicalName(),
